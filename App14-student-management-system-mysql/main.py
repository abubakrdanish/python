...
class DatabaseConnection:
    def __init__(self, host="localhost", user="root", password="1234", database="school"):
        self.host = host
        self.user = user
...
class MainWindow(QMainWindow):
    ...
    def insert(self):
        dialog = InsertDialog(self.db_connection, self)
        dialog.exec()
        self.load_data()

    def search(self):
        dialog = SearchDialog(self.db_connection, self)
        dialog.exec()

    def edit(self):
        if self.table.currentRow() < 0:
            self.statusbar.showMessage("Select a record to edit")
            return
        dialog = EditDialog(self.db_connection, self)
        dialog.exec()

    def delete(self):
        if self.table.currentRow() < 0:
            self.statusbar.showMessage("Select a record to delete")
            return
        dialog = DeleteDialog(self.db_connection, self)
        dialog.exec()
...
class EditDialog(QDialog):
    def __init__(self, db_connection, main_window):
        super().__init__()
...
class SearchDialog(QDialog):
    ...
    def search(self):
        ...
        if rows:
            for row in rows:
                items = self.main_window.table.findItems(row[1], Qt.MatchFlag.MatchFixedString)
                for item in items:
